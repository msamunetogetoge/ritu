...
export default function BillingPage(): JSX.Element {
  ...
  const handleUpgrade = async () => {
    ...
    const confirmed = confirm("月額500円でプレミアムプランに登録しますか？ (Mock Payment)");
    if (confirmed) {
        // Mock upgrade
        try {
    ...
    <div className="main-scroll p-4">
      <div className="billing-card">
        ...
        <div className="current-plan">
            <p>現在のプラン: <strong>{isPremium ? "プレミアム" : "フリー"}</strong></p>
            {!isPremium && <p className="limit-note">フリープランではルーティーンは2つまで作成可能です。</p>}
        </div>

        {isPremium ? (
            <div className="premium-actions">
                <p>あなたはプレミアム会員です。すべての機能をご利用いただけます。</p>
                <button type="button" className="btn-secondary" onClick={handleCancel}>解約する</button>
            </div>
        ) : (
            <div className="upgrade-actions">
                ...
                <button type="button" className="btn-primary" onClick={handleUpgrade}>プレミアムに登録する</button>
            </div>
        )}
      </div>
